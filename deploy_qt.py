...
# call generator function that creates a DLLInfo object (with sensible defaults for everything except module, source_dir & base_name)
    self.dlls = [
            ...
            CreateDLLEntry( comp_multimedia, 'plugins', 'audio/qtaudio_wasapi' , platforms=['windows'] ),
            CreateDLLEntry( comp_multimedia, 'plugins', 'audio/qtaudio_windows' , platforms=['windows'] ),
...
#----------------------------------------------------------------------
class Qt51210os(Qt592os):
  """
  Tested on Ubuntu 1804 only
  """
  def __init__(self):
    Qt592os.__init__(self)
    ...
    comp_base = []
    comp_angle = ['angle']
    self.dlls.append(CreateDLLEntry( comp_quick,   'lib', 'Qt5MultimediaQuick'))
    self.dlls.append(CreateDLLEntry( comp_quick,   'lib', 'Qt5QuickShapes', platforms=['linux']))
    self.dlls.append(CreateDLLEntry( comp_quick,   'lib', 'Qt53DQuick', platforms=['linux']))
    self.dlls.append(CreateDLLEntry( comp_quick,   'lib', 'Qt53DAnimation', platforms=['linux']))
    self.dlls.append(CreateDLLEntry( comp_position, 'lib', 'Qt5PositioningQuick'))

    self.dlls.append(CreateDLLEntry( comp_base,    'lib', 'icui18n', platforms=['linux']))
    self.dlls.append(CreateDLLEntry( comp_base,    'lib', 'icuuc', platforms=['linux']))
    self.dlls.append(CreateDLLEntry( comp_base,    'lib', 'icudata', platforms=['linux']))
...
#----------------------------------------------------------------------
class Qt5152os(Qt5140os):
  def __init__(self):
    Qt5140os.__init__(self)
  ...
  def getName(self):
    return 'Qt 5.15.2 Open Source'

QT_VERSIONS   = [Qt51210os(), Qt5126os(), Qt5125os(), Qt560os(), Qt590os(), Qt591os(), Qt592os(), Qt531os(), Qt530os(), Qt520os(), Qt511os(), Qt5140os(), Qt5152os()]
...
#                        help='select the platform. Host platform is the default')
    ...
    # name of dewetron build qt artefacts
    ...
    if os.path.exists(args.qt_directory):
      ...
      if not os.path.exists(qt_build_directory):
        qt_build_directory = args.qt_directory
    else:
